 ...
 * Database Component. This class implements IRegisterWindow.
 * 
...
public class Register implements IRegisterWindow {
	...
	/**
	 * this hashtable is used map the searc id with the List of SearchPattern
	 * class object.
	 */
	private Hashtable<String, List<SearchPattern>> searchHash = null;
	/**
	 * this hashtable is used map the formPattern id with the FormPattern class
	 * object.
	 */
	private Hashtable<String, FormPattern> formPatternHash = null;
	/**
	 * this hashtable map string element name with the hashtable
	 * formPatternHash.That hashtable maps element id with the object of the
	 * class FormPattern.
	 */
	private Hashtable<String, Hashtable> baseFormPatternHash = null;
	/**
	 * this hashtable map string element name with the hashtable
	 * dataPatternHash.That hashtable maps dataPattern id with the object of the
	 * class DataPattern.
	 */
	private Hashtable<String, Hashtable> baseDataPatternHash = null;
	/**
	 * this hashtable maps dataPattern id with the object of the class
	 * DataPattern.
	 */
	// This is changed as periviously it is taking only datapattern object,as
	// implementation is changed now will take linkedlist
	...
	/**
	 * this is used to persists the appliction pattern name for the
	 * applicationPatternHash hashtable.
	 */
	...
	/**
	 * This method will be called by all base windows to register with DB
	 * component.Receives an XML document as input and will parse this
	 * information using parsePattern() method.
	 * 
	 * @param patternXML
	 *            This XML document that contains all the elements in form
	 *            pattern and its mappings.
	 * @param applicationName
	 *            This is the name of the registering application.
	 * @throws DBComponentException
	 * @returns void
	 */
	public void registerBaseWindow(Document patternXML, String applicationName)
			throws DBComponentException {
		...
		if (patternRoot.getName().trim()
			...
			if (null != applicationPatternHash.get(applicationName)) {
				applicationPatternList = applicationPatternHash
						.get(applicationName);
				applicationPatternHash.remove(applicationName);
				applicationPatternList.add(patternRoot
						.attributeValue(RegisterConstants.REG_NAME_ATTR));

				applicationPatternHash.put(applicationName,
				...
				applicationPatternHash.put(applicationName,
						applicationPatternList);
				// DBComponent.logger.log(0,"applicationPatternHash = " +
				// applicationPatternHash);
			}
	...
	/*
	 * It parses each element received from registerWindow Method. It will
	 * persist the information regarding each element of base window in memory
	 */
	private void parsePattern(Element patternRoot, boolean parseSubElements)
			throws DBComponentException {
		...
		if (null == patternRoot.attribute(RegisterConstants.REG_ID_ATTR)) {
			...
			elementId = patternRoot
					.attributeValue(RegisterConstants.REG_ID_ATTR);
		}
		if (null == patternRoot.attribute(RegisterConstants.REG_NAME_ATTR)) {
			elementName = "";
		} else {
			elementName = patternRoot
					.attributeValue(RegisterConstants.REG_NAME_ATTR);
		}
		...
		if (patternRoot.isRootElement()) {
			...
			if (null != patternRoot.attribute(RegisterConstants.REG_SEARCH_TAG)) {
				searchId = patternRoot
						.attributeValue(RegisterConstants.REG_SEARCH_TAG);
			}

			if ((null != patternRoot.elements())
					&& (patternRoot.elements().size() > 0)) {

				dataPatternId = parentId + DBConstants.DB_SPACE + xmlTagName
						+ DBConstants.DB_SPACE + elementId;
				List sublelemts = patternRoot.elements();
				// Implementation changed for saving data in the multiple
				// tables.
				...
				for (int i = 0; i < sublelemts.size(); i++) {
					...
					if (null != subChild.elements()
							&& subChild.getName().equalsIgnoreCase(
									RegisterConstants.REG_ID_FIELDMAP)) {
						List childs = subChild.elements();
						for (int j = 0; j < childs.size(); j++) {
							Element subChildElement = (Element) childs.get(j);
							if (null != subChildElement
									.attributeValue(RegisterConstants.REG_DATA_PATTERN)) {
								databaseName = subChildElement
										.attributeValue(RegisterConstants.REG_DATA_PATTERN);
								System.out
										.println(" ***** INSIDE REGISTER CLASS DATABASENAME = "
												+ databaseName);
								fieldNameType = subChildElement
										.attributeValue(RegisterConstants.REG_DATA_FIELD);
								System.out
										.println(" *****INSIDE REGISTER CLASS FIELDNAMETYPE = "
												+ fieldNameType);
								if (null != fieldNameType) {
									int indexOfColon = fieldNameType
											.indexOf(DBConstants.DB_COLON);
									fieldName = fieldNameType.substring(0,
											indexOfColon);
									fieldType = fieldNameType
											.substring(indexOfColon + 1);
									fieldType = fieldType.trim();
								}

								if (null != subChildElement
										.attribute(RegisterConstants.REG_DATA_TABLE)) {
									tableName = subChildElement
											.attributeValue(RegisterConstants.REG_DATA_TABLE);
								}
								...
								dataPattern.setTableName(tableName
										.toLowerCase());
				...
				// if(null !=
				// patternRoot.attributeValue(RegisterConstants.REG_DATA_PATTERN))
				// {
				// databaseName =
				// patternRoot.attributeValue(RegisterConstants.REG_DATA_PATTERN);
				// dataPatternId = parentId + DBConstants.DB_SPACE + xmlTagName
				// + DBConstants.DB_SPACE + elementId;
				// fieldNameType =
				// patternRoot.attributeValue(RegisterConstants.REG_DATA_FIELD);
				// if(null != fieldNameType)
				// {
				// int indexOfColon =
				// fieldNameType.indexOf(DBConstants.DB_COLON);
				...
				//
				// if(null !=
				// patternRoot.attribute(RegisterConstants.REG_DATA_TABLE))
				// {
				// tableName =
				// patternRoot.attributeValue(RegisterConstants.REG_DATA_TABLE);
				// }
			...
			if (xmlTagName.equalsIgnoreCase(RegisterConstants.REG_SEARCH_TAG)) {
				...
				for (int searchCount = 0; searchCount < searchElementList
						.size(); searchCount++) {
					Element searchElement = (Element) searchElementList
							.get(searchCount);
					if (null != searchElement
							.attribute(RegisterConstants.REG_NAME_ATTR)) {
						searchId = searchElement
								.attributeValue(RegisterConstants.REG_NAME_ATTR);
						List queryElementList = searchElement
								.elements(RegisterConstants.REG_QUERY_ATTR);
						for (int queryCount = 0; queryCount < queryElementList
								.size(); queryCount++) {
							srchPattern = new SearchPattern();
							Element queryElement = (Element) queryElementList
									.get(queryCount);
							if (null != queryElement
									.attribute(RegisterConstants.REG_FIELD_NAME_ATTR)) {
								elementToSearch = queryElement
										.attributeValue(RegisterConstants.REG_FIELD_NAME_ATTR);
							}
							if (null != queryElement
									.attribute(RegisterConstants.REG_OPER_ATTR)) {
								joinOperator = queryElement
										.attributeValue(RegisterConstants.REG_OPER_ATTR);
							}
							if (null != queryElement
									.attribute(RegisterConstants.REG_COND_ATTR)) {
								conditionOperator = queryElement
										.attributeValue(RegisterConstants.REG_COND_ATTR);
							}
							if (null != queryElement
									.attribute(RegisterConstants.REG_FIELD_VALUE_ATTR)) {
								mappingValue = queryElement
										.attributeValue(RegisterConstants.REG_FIELD_VALUE_ATTR);
							}
		 ...
		 * DBComponentException(e.getMessage())); }
		 */
	}

	/**
	 * This method will be called by all zk windows to register with DB
	 * component.Receives an zk document as input and will parse this
	 * information using parseZKPattern() method. This API is invoked by
	 * getConnection() API of DBComponent
	 * 
	 * @param patternZUL
	 *            This ZUL document that contains all the elements in form
	 *            pattern and its mappings.
	 * @param applicationName
	 *            This is the name of the registering application.
	 * @throws DBComponentException
	 ...
	 * @author Ravneet
	 */
	public void registerZKWindow(Document patternZUL, String applicationName)
			throws DBComponentException {
		formPatternHash = new Hashtable<String, FormPattern>();
		dataPatternHash = new Hashtable<String, LinkedList<DataPattern>>();
		// DBComponent.logger.log(0,"registering started");
	 ...
	 * register with DB component.It will persist the information regarding each
	 * element of zk window in memory
	 * 
	 * @param patternRoot
	 *            : The root element which represents the root tag of zul file :
	 *            zk
	 * @param parseSubElements
	 *            : This value will be set to true if subElemnets has to be
	 *            parsed
	 * @throws DBComponentException
	 * @returns void
	 * @author Ravneet
	 **/
	private void parseZULPattern(Element patternRoot, boolean parseSubElements)
			throws DBComponentException {
	 ...
	 * 
	 * @returns Hashtable<String, List<String>> which maps application name to
	 *          application pattern list.
	 */
	...
	/**
	 * This method is used to get the hashtable baseFormPatternHash which maps
	 * string to hashtable formPatternHash, this formPatternHash hashtable maps
	 * elementid to the FormPattern class object.
	 * 
	...
	/**
	 * This method is used to set the hashtable baseFormPatternHash which maps
	 * string to hashtable formPatternHash, this formPatternHash hashtable maps
	 * elementid to the FormPattern class object.
	 * 
	...
	/**
	 * This method is used to get the hashtable dataPatternHash which maps id of
	 * DataPattern to to the object of the DataPattern class. DataPattern class
	 * object has all information related to datapattern.
	 * 
	 * @returns void
	 */
	...
	/**
	 * This method is used to set the hashtable dataPatternHash which maps id of
	 * DataPattern to the object of the DataPattern class. DataPattern class
	 * object has all information related to datapattern.
	 * 
	 * @param dataPatternHash
	 * @returns void
	 */
	public void setDataPatternHash(
			Hashtable<String, LinkedList<DataPattern>> dataPatternHash) {
		this.dataPatternHash = dataPatternHash;
	}

	/**
	 * This method is used to get the hashtable formPatternHash which maps id of
	 * FormPattern to the object of the FormPattern class. FormPattern class
	 * object has all information related to FormPattern.
	 * 
	...
	/**
	 * This method is used to set the hashtable formPatternHash which maps id of
	 * FormPattern to the object of the FormPattern class. FormPattern class
	 * object has all information related to FormPattern.
	 * 
	 * @param dataPatternHash
	 * @returns void
	 */
	public void setFormPatternHash(
			Hashtable<String, FormPattern> formPatternHash) {
		this.formPatternHash = formPatternHash;
	}

	/**
	 * This method is used to get the hashtable searchHash which maps search id
	 * to the search pattern list.This list has the objects of the class
	 * SearchPattern in it.
	 * 
	...
	/**
	 * This method is used to set the hashtable searchHash which maps search id
	 * to the search pattern list.This list has the objects of the class
	 * SearchPattern in it.
	 * 
	 * @param searchHash
	...
	/**
	 * This method is used to get the applicationPatternList which has the
	 * application pattern name in it.
	 * 
	 * @returns List<String>
	 */
	...
	/**
	 * This method is used to set the applicationPatternList which the
	 * application pattern name in it.
	 * 
	...
	/**
	 * This method is used to get the hashtable which maps string element name
	 * to hashtable dataPatternHash.The dataPatternHash maps dataPattern id with
	 * the object of the class DataPattern.
	 * 
	 * @returns Hashtable<String, Hashtable>
	 */
